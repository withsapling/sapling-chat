...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function addMessage(text, images = [], isUser = false) {
    ...
    if (isUser) {
      ...
      if (text) {
        // If we already have text, render it immediately
  ...
  async function sendMessage(message) {
    ...
    try {
      ...
      // Add a class to mark it as streaming
      ...
      // Create a transformer to accumulate markdown before rendering
      const markdownAccumulator = new TransformStream({
        transform(chunk, controller) {
          markdownBuffer += chunk;
          controller.enqueue(markdownBuffer);
        },
      });

      // Process the stream: text -> accumulated markdown -> rendered HTML
      const markdownStream = await textStream
        .pipeThrough(markdownAccumulator)
        .pipeThrough(
          new TransformStream({
            async transform(accumulatedMarkdown, controller) {
              try {
                const html = await renderMarkdown(accumulatedMarkdown);
                controller.enqueue(html);
              } catch (error) {
                console.error("Rendering error:", error);
                controller.enqueue(accumulatedMarkdown);
              }
            },
          })
        );

      // Pipe the markdown stream to the DOM
      const writer = new WritableStream({
        ...
        close() {
          ...
          saveChatHistory(markdownBuffer, false);
        },
      });

      // Start streaming the content
      await markdownStream.pipeTo(writer);
    } catch (error) {
